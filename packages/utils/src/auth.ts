...
export function alwaysFailAuth(request: FastifyRequest, reply: FastifyReply, done: any) {
  done(AyazmoError({
    statusCode: 401,
...
export function getAuthProviderFunction(app: AyazmoInstance, providerName: string): any {
  const providerFunction = app[providerName];
  if (app.hasDecorator(providerName)) {
