...
export class AyazmoPublisher {
  private readonly publisher: Queue
  private readonly flowProducer?: FlowProducer
  public isFlow: boolean = false
  private readonly app: AyazmoInstance
  private readonly config: AppConfig
  private readonly eventPublishersMap = new Map<string, AyazmoQueue[]>()
  ...
  setupEventHandlers (queue: Queue, config?: AyazmoQueue): void {
    allQueueEvents.forEach((event) => {
      const configEvent = config?.events?.[event] ?? null
      if (configEvent != null) {
        // Use the configured event handler
        queue.on(event as any, configEvent)
      }
    })
  }
  ...
  setEventPublishersMap (publishers: AyazmoQueue[] = []): void {
  ...
  async publish (event: string, data: any | any[]): Promise<void> {
    ...
    if (this.isFlow && (this.flowProducer != null)) {
      const jobPromises = publishers.map(async (publisher: AyazmoQueue) => {
        ...
        const transformedData = (publisher.transformer != null) ? await publisher.transformer(data, event, this.app) : data
...
export class AyazmoWorker {
  ...
  private readonly app: AyazmoInstance
  private readonly config: AppConfig
  private readonly eventHandlers: Map<string, Set<(...args: any[]) => void>>
  ...
  setupEventHandlers (worker: Worker, config: AyazmoWorkerType) {
    allWorkerEvents.forEach((event) => {
      if ((config.events != null) && config.events[event]) {
        // Use the configured event handler
...
export class RedisEventEmitter extends BaseEventEmitter {
  private readonly publisher: AyazmoPublisher
  private readonly workers: AyazmoWorker
  ...
  constructor (app: AyazmoInstance, config: AppConfig) {
    ...
    if ((config.app?.emitter?.workers) != null) {
